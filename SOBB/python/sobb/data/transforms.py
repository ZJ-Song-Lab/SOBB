...
from sobb.utils.registry import build_from_cfg,TRANSFORMS
from sobb.models.boxes.box_ops import rotated_box_to_poly_np,poly_to_rotated_box_np,norm_angle
from sobb.models.boxes.iou_calculator import bbox_overlaps_np
...
@TRANSFORMS.register_module()
class RandomRotateRange:
    """Arbitrary-angle random rotation in [-max_deg, +max_deg] (default +-30).

    Rotates the image and all box types (hboxes, rboxes, polys) together so
    that training-time geometry matches the paper protocol (+-30 deg random
    rotation, independent of the 90 deg RandomRotateAug).
    """
    def __init__(self, max_deg=30.0, prob=0.5):
    ...
    def _rotate_rboxes(self, bboxes, center, angle, w, h):
        import numpy as _np
        bboxes = bboxes.copy()
        cx, cy = center
        # Negate angle to match Pillow's CCW rotation in image coords
        rad = _np.deg2rad(-angle)
        cos, sin = _np.cos(rad), _np.sin(rad)
        x, y, ww, hh, th = (bboxes[:, i] for i in range(5))
        dx = x - cx
        dy = y - cy
        nx = cos * dx - sin * dy + cx
        ny = sin * dx + cos * dy + cy
    ...
    def _rotate_polys(self, polys, center, angle):
        import numpy as _np
        cx, cy = center
        # Pillow rotates CCW for positive angles, but image coords have y-down
        # So we negate the angle for box rotation to match image rotation
        rad = _np.deg2rad(-angle)
        cos, sin = _np.cos(rad), _np.sin(rad)
        # Convert to numpy array for proper slicing
        polys_arr = _np.array(polys, dtype=_np.float32) if not isinstance(polys, _np.ndarray) else polys.copy()
        xs, ys = polys_arr[:, 0::2], polys_arr[:, 1::2]
        dx = xs - cx
        dy = ys - cy
        polys_arr[:, 0::2] = cos * dx - sin * dy + cx
        polys_arr[:, 1::2] = sin * dx + cos * dy + cy
        return polys_arr

    def _rotate_hboxes(self, bboxes, center, angle):
        import numpy as _np
        polys = _np.zeros((bboxes.shape[0], 8))
        polys[:, 0::2] = bboxes[:, 0]
        polys[:, 2::2] = bboxes[:, 2]
        polys[:, 4::2] = bboxes[:, 2]
        polys[:, 6::2] = bboxes[:, 0]
        ...
        dx = polys[:, 0::2] - cx
        dy = polys[:, 1::2] - cy
        polys[:, 0::2] = cos * dx - sin * dy + cx
        polys[:, 1::2] = sin * dx + cos * dy + cy
        xs = polys[:, 0::2]
        ys = polys[:, 1::2]
        out = bboxes.copy()
        out[:, 0] = xs.min(axis=1)
    ...
    def __call__(self, image, target=None):
        ...
        if _r.random() >= self.prob:
            return image, target
        angle = _r.uniform(-self.max_deg, self.max_deg)
        w, h = image.size
        center = (w / 2.0, h / 2.0)
        # Rotate image with expand -> new canvas size
        image = self._rotate_image(image, angle)
        nw, nh = image.size
        # Translation offset: new center minus old center
        # After expand, PIL places the rotated image so its center is at
        # (nw/2, nh/2). The old center was at (w/2, h/2).
        # Box coordinates were relative to old center, so we shift by:
        dx = (nw - w) / 2.0
        dy = (nh - h) / 2.0
        if target is not None:
            for key in ["bboxes", "hboxes", "rboxes", "polys",
                        "hboxes_ignore", "polys_ignore", "rboxes_ignore"]:
                if key not in target:
                    continue
                b = target[key]
                if "rboxes" in key and b.ndim >= 2:
                    b = self._rotate_rboxes(b, center, angle, w, h)
                    # Apply canvas-expansion translation
                    b[:, 0] += dx
                    b[:, 1] += dy
                    target[key] = b
                elif "polys" in key and b.ndim >= 2:
                    b = self._rotate_polys(b, center, angle)
                    # Apply canvas-expansion translation to all x,y coords
                    b[:, 0::2] += dx
                    b[:, 1::2] += dy
                    target[key] = b
                elif b.ndim >= 2:
                    b = self._rotate_hboxes(b, center, angle)
                    b[:, 0] += dx
                    b[:, 1] += dy
                    b[:, 2] += dx
                    b[:, 3] += dy
            ...
            # Update image size for downstream transforms
            target["img_size"] = (nw, nh)
            if "img_shape" in target:
                target["img_shape"] = (nh, nw, target["img_shape"][2]) if len(target.get("img_shape", ())) == 3 else (nh, nw)
        return image, target


@TRANSFORMS.register_module()
class IndependentHVFlip:
    """Apply horizontal and vertical flips independently (each with prob 0.5).

    This matches the paper protocol of independent horizontal / vertical flips.
    """
    def __init__(self, prob_h=0.5, prob_v=0.5):
        self.prob_h = float(prob_h)
        self.prob_v = float(prob_v)
        self._h = RotatedRandomFlip(prob=self.prob_h, direction="horizontal")
        self._v = RotatedRandomFlip(prob=self.prob_v, direction="vertical")

    def __call__(self, image, target=None):
        image, target = self._h(image, target)
        image, target = self._v(image, target)
        return image, target


@TRANSFORMS.register_module()
class ConsistencyPerturbation:
    """Weak perturbation for candidate-set consistency (paper Eq 34-35).

    With equal probability, applies one of:
    (1) Rotation from [-3, 3] degrees
    (2) Independent H/V translation from [-2, 2] pixels
    (3) Multiplicative unit-mean Gamma noise (shape=100, scale=0.01)
        with identity geometric transform (boxes unchanged)
    """
    def __init__(self):
        import numpy as np
        self._np = np
    ...
    def __call__(self, results):
        ...
        if choice == 0:
            ...
            results['img'] = self._rotate_image(results['img'], angle)
            if 'gt_bboxes' in results:
                results['gt_bboxes'] = self._rotate_bboxes(
                    results['gt_bboxes'], angle,
                    results['img'].shape[1], results['img'].shape[0])
            results['perturbation_type'] = 'rotation'
            results['perturbation_angle'] = angle
        elif choice == 1:
            tx = np.random.uniform(-2.0, 2.0)
            ty = np.random.uniform(-2.0, 2.0)
            results['img'] = self._translate_image(results['img'], tx, ty)
            if 'gt_bboxes' in results:
                bboxes = results['gt_bboxes']
                bboxes[:, 0] += tx
                bboxes[:, 1] += ty
                results['gt_bboxes'] = bboxes
            results['perturbation_type'] = 'translation'
        else:
            img = results['img'].astype(np.float32)
            noise = np.random.gamma(100, 0.01, size=img.shape).astype(np.float32)
            results['img'] = np.clip(img * noise, 0, 255).astype(np.uint8)
            results['perturbation_type'] = 'gamma_noise'
    ...
    def _rotate_bboxes(self, bboxes, angle, img_w, img_h):
        ...
        dx = bboxes[:, 0] - cx
        dy = bboxes[:, 1] - cy
        new_bboxes[:, 0] = dx * cos_a - dy * sin_a + cx
        new_bboxes[:, 1] = dx * sin_a + dy * cos_a + cy
        if bboxes.shape[1] >= 5:
            new_bboxes[:, 4] = bboxes[:, 4] + rad
        return new_bboxes
    ...
    def __repr__(self):
        return 'ConsistencyPerturbation()'


@TRANSFORMS.register_module()
class ContinuousResize:
    """Continuous multiscale resize with scale factor sampled from [scale_min, scale_max].

    Matches paper protocol: 'input images are randomly resized with a scale factor
    sampled from [0.8, 1.2]' (Section 5.1.2).
    """
    def __init__(self, scale_range=(0.8, 1.2), base_size=800, max_size=1333):
    ...
    def __call__(self, image, target=None):
        import random as _r
        from sobb.models.boxes.box_ops import rotated_box_to_poly_np, poly_to_rotated_box_np
        scale = _r.uniform(self.scale_min, self.scale_max)
        target_size = int(round(self.base_size * scale))
        target_size = max(32, min(target_size, self.max_size))
        w, h = image.size
        if w <= h:
            new_w = target_size
        ...
        if max(new_w, new_h) > self.max_size:
            ratio = self.max_size / max(new_w, new_h)
        ...
        if target is not None:
            for key in ["bboxes", "hboxes", "rboxes", "polys",
                ...
                if b is None:
                    continue
                # Handle list input (some pipelines store lists)
                if isinstance(b, list):
                    b = np.array(b, dtype=np.float64)
                if not hasattr(b, 'ndim') or b.ndim != 2:
                    continue
                # Skip empty boxes (shape (0, 5) etc.)
                if b.shape[0] == 0:
                    continue
                # Convert rboxes to polys for resizing (matches RotatedResize)
                is_rbox = "rboxes" in key
                if is_rbox:
                    b = rotated_box_to_poly_np(b)
                b = b.astype(np.float64)
                b[:, 0::2] *= new_w / w
                b[:, 1::2] *= new_h / h
                b[:, 0::2] = np.clip(b[:, 0::2], 0, new_w - 1)
                b[:, 1::2] = np.clip(b[:, 1::2], 0, new_h - 1)
                if is_rbox:
                    b = poly_to_rotated_box_np(b)
                target[key] = b
            target["img_size"] = (new_w, new_h)
            target["scale_factor"] = np.array([new_w / w, new_h / h], dtype=np.float32)
            target["pad_shape"] = (new_h, new_w)
            target["keep_ratio"] = True
        return image, target

    def __repr__(self):
        return (f'ContinuousResize(scale_range=({self.scale_min}, {self.scale_max}), '
                f'base_size={self.base_size}, max_size={self.max_size})')
