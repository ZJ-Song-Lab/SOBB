...
from sobb.utils.registry import LOSSES
...
class CrossEntropyLoss(nn.Module):
    ...
    def __init__(self,reduction='mean',use_bce=False, loss_weight=1.0,
        ...
        self.loss_weight = loss_weight
        # S0-3: Accept use_sigmoid as an alias for use_bce so that configs
        # using use_sigmoid=True (the mmdet convention) work correctly.
        # If both are specified, use_sigmoid takes precedence.
        if use_sigmoid is not None:
            self.use_bce = use_sigmoid
        else:
            self.use_bce = use_bce
        # Expose use_sigmoid as an attribute for downstream code that
        # checks self.loss_cls.use_sigmoid
        self.use_sigmoid = self.use_bce
