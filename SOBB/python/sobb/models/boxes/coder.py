from sobb.ops.bbox_transforms import *
from .box_ops import delta2bbox,bbox2delta,delta2bbox_rotated,bbox2delta_rotated
from .sobb_ops import sobb_decode, sobb_encode
from sobb.utils.registry import BOXES
...
class SOBBBBoxCoder:
    ...
    Encodes/Decodes the SOBB 7-parameter vector (dx, dy, dw, dh, t_s, s1, s2).
    The means/stds (length-4) apply only to the HBB deltas (dx, dy, dw, dh).
    The shape target t_s and candidate scores s1, s2 are not normalized.
    """
    ...
    def encode(self, bboxes, gt_bboxes):
        ...
        """
        assert bboxes.size(0) == gt_bboxes.size(0)
        return sobb_encode(bboxes, gt_bboxes, self.means, self.stds)

    def decode(self, bboxes, pred_bboxes):
        """
        bboxes: HBB [N, 4]
        pred_bboxes: SOBB deltas [N, 7]
        """
        assert pred_bboxes.size(0) == bboxes.size(0)
        return sobb_decode(bboxes, pred_bboxes, self.means, self.stds)
