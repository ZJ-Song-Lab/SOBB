...
import pickle
import datetime
from sobb.config import get_cfg,save_cfg
from sobb.utils.visualization import visualize_results
from sobb.utils.registry import build_from_cfg,MODELS,SCHEDULERS,DATASETS,HOOKS,OPTIMS
from sobb.models.boxes.assigner import set_sala_epoch
from sobb.config import get_classes_by_name
from sobb.utils.general import build_file, current_time, sync,check_file,check_interval,parse_losses,search_ckpt
from sobb.data.devkits.data_merge import data_merge_result
import os
import shutil
from tqdm import tqdm
...
class Runner:
    def __init__(self):
        ...
        if self.resume_path == "__no_resume__":
    ...
    def test(self):
        ...
        if self.test_dataset is None:
            ...
            # S0-1: Evaluate test results to produce AP/AR metrics.
            # Previously test() only saved predictions without computing
            # metrics. Now call evaluate() just like val() does, so that
            # the reproduce script's --task test produces real metrics.
            if hasattr(self.test_dataset, 'evaluate'):
                eval_results = self.test_dataset.evaluate(
                    results, self.work_dir, self.epoch,
                    logger=self.logger)
                self.logger.log(eval_results, iter=self.iter)
                # Save evaluation results as JSON for the seed aggregator
                import json as _json
                eval_json = build_file(self.work_dir,
                    f"test/eval_{self.epoch}.json")
                _serializable = {}
                for k, v in eval_results.items():
                    try:
                        _serializable[k] = float(v)
                    except (TypeError, ValueError):
                        _serializable[k] = str(v)
                with open(eval_json, 'w') as _f:
