# Using SOBB with SSDD/SSDD+
Using SOBB with SAR Ship Detection Dataset (SSDD/SSDD+).
## Data Preparing
...
└── Official-SSDD-OPEN
      ...
      └──RBox_SSDD/voc_style
            ...
            └──Annotations_test
```
Only the 8 folders above(JPEGImages_train, JPEGImages_test, Annotations_train, Annotations_test) are used in SOBB.
## Data Preprocessing
...
target_dataset_path='/path/to/your/processed_SSDD_plus/'
...
```
For the way of configuring the processed SSDD/SSDD+ dataset in the model config file, please refer to `$SOBB_PATH$/projects/s2anet/configs/s2anet_r50_fpn_1x_ssdd.py`/`$SOBB_PATH$/projects/s2anet/configs/s2anet_r50_fpn_1x_ssdd_plus.py`:
```
dataset = dict(
    ...
)
...
## Data Postprocessing
The Runner.val() in SOBB will automatically calculate the AP50 of SSDD/SSDD+ test set. 
