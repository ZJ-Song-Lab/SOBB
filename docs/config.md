...
```python
# main.py
from SOBB.config import init_cfg
init_cfg('cfg.yaml')
```
Then you can get the configuration var in other files:
```python
# model.py
from SOBB.config import get_cfg
cfg = get_cfg()
...
```python
# model.py
from SOBB.config import save_cfg, print_cfg
...
Please refer to `[ROOT]/python/SOBB/config/config.py` and `[ROOT]/tests/test_config` for more details.